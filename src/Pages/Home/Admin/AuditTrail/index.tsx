...
const AuditTrailTab: React.FC = () => {
  ...
  const groupEntriesByTimePeriod = (entries: AuditTrailEntry[]): Record<TimeGroup, AuditTrailEntry[]> => {
    const now = new Date();
    ...
    <div className="flex flex-col w-full">
      ...
      {Object.entries(groupedEntries).map(([groupName, entries]) => (
        <div key={groupName} className="w-full">
          ...
          {entries.map((entry) => (
            <div key={entry.id} className="flex items-start p-4 border bg-[#F8F8F8] mb-[14px] rounded-[6px] hover:bg-gray-50">
              ...
              <div className="flex flex-col">
                <span className="text-[#19181966] text-sm font-medium">{formatTime(entry.timestamp)}</span>
                <span className="text-sm text-[#19181980]">
                  <span className="font-medium text-[#191819]">{entry.userName}</span> {entry.action} a{' '}
