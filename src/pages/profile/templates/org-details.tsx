...
const EditOrganizationDetails: React.FC = () => {
  ...
  useEffect(() => {
    if (company?.value?.data) {
      form.setValue("name", company.value.data.name);
      form.setValue("industry_type", company.value.data.industry_type);
      form.setValue("size", company.value.data.size);
      form.setValue("address", company.value.data.address);
    }
